...
const handleError = (error) => {
    ...
    dialog_header.insertAdjacentHTML('afterbegin',"<h4>Unexpected error occurred</h4>");
    dialog_article.replaceChildren();
    dialog_article.insertAdjacentHTML('afterbegin',"<h4>" + error.name + ": " + error.message + "</h4>");
...
const callAPI = async (endpoint, method, data) => {
        ...
        if (expiredToken(access_token)) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    const error = result.sqlerrm || result.cause || result['o:errorCode'] + " - " + result.message;
    ...
    const path = endpoint.replace(":ID",bodydata.websiteid).replace(":PAGE",bodydata.articleid);
